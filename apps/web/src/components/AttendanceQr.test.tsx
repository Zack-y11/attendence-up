...
const qrTitle = 'QR code for the public attendance page';
const qrCaption = 'Scan to open the same check-in page as Copy link.';

function qrPaths(value: string) {
  const html = renderToStaticMarkup(
    <QRCodeSVG value={value} size={176} title={qrTitle} {...attendanceQrOptions} />,
  );
...
describe('public attendance QR', () => {
  it('encodes the same /a/{token} URL that Copy link uses', () => {
    ...
    expect(html).toContain(qrCaption);
  });
});
